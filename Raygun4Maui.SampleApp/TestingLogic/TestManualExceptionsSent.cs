...
{
    internal class TestManualExceptionsSent(IRaygunMauiUserProvider userProvider)
    {
        ...
        {
            userProvider.SetUser(new RaygunIdentifierMessage("user1@email.com"));
            await RaygunMauiClient.Current.SendAsync(GenerateException("TestUniqueUserTracking"));

            userProvider.SetUser(new RaygunIdentifierMessage("user2@email.com")
            {
                IsAnonymous = false,
            ...
            userProvider.SetUser(new RaygunIdentifierMessage(null) { IsAnonymous = true });
        }
